...
class MobileNetV3_Small(nn.Module):
    def __init__(self, num_classes=1000, act=nn.Hardswish):
        ...
        self.linear4 = nn.Linear(1280, num_classes=4)
        self.init_params()
    ...
    def forward(self, x):
        ...
        out = self.gap(out).flatten(1)  # Seharusnya output [batch_size, 576]

        print("Output sebelum linear3:", out.shape)  # Debugging

        out = self.drop(self.hs3(self.bn3(self.linear3(out))))

        print("Output sebelum nn.Linear:", out.shape)  # Debugging

        return self.linear4(out)  # Harus [batch_size, num_classes]
